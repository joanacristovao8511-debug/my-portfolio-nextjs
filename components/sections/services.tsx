export function ServicesSection(){
  ...
  return <section className="mx-auto max-w-7xl px-6 py-16" id="services">
    ...
    <div className="mt-8 grid gap-6 md:grid-cols-3">{services.map((s)=><article key={s.title} className="rounded-3xl border p-6"><h3 className="text-xl font-bold">{s.title}</h3><p className="mt-3 text-muted">{s.text}</p></article>)}</div>
  </section>;
}
